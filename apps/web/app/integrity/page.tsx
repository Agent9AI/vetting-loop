...
export const metadata: Metadata = {
  title: "Integrity Index | The Vetting Loop",
...
export default function IntegrityIndexPage() {
  ...
  const rows: IndexRow[] = profiles.map((p) => {
    const a2024 = p.appointments.find((a) => a.from.startsWith("2024-0"));
  ...
  return (
    <main>
      <PageHeader
        crumbs={[{ label: "Home", href: "/" }, { label: "Civic Tech Tools", href: "/civic-tech" }, { label: "Integrity Index" }]}
        title="Integrity Index"
        lede={
          <>
            A {LOOKBACK_YEARS}-year record of integrity findings for public officers, drawn from the Ethics and
            Anti-Corruption Commission, the Office of the Auditor-General, parliamentary vetting and Mzalendo. Rated
            ahead of the {formatDate(DEFAULT_AS_OF)} general election. Most nominees carry no adverse record.
          </>
        }
      />
      <MockBanner />
      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        <h2 className="mb-1 text-xl font-bold text-mz-red">2024 Cabinet vetting cohort</h2>
        <p className="mb-6 text-sm text-mz-muted">
          The 20 nominees vetted by the Committee on Appointments in July–August 2024: 19 approved, 1 rejected. Placeholder
          roster until the 2027 candidates are known. <Link href="/integrity/roster" className="mz-link">Roster status</Link>
        </p>
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="mz-stat">
            ...
            <p className="text-xs text-mz-muted">2024 cohort</p>
          </div>
          <div className="mz-stat">
